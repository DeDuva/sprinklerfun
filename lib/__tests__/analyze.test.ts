...
describe("recommendStationDelays", () => {
  ...
  it("reproduces the measured production result on a whole-day fixture", () => {
    // A full day laid out the way the two controllers actually behave: timer 1
    // runs its stations back to back, timer 2 inserts a minute of dead time
    // between them AND runs six minutes long overall. The expected values below
    // are the ones measured from the real meter on 2026-08-28 (see
    // docs/TECHNICAL_DESIGN.md §7); the fixture is generated by
    // scripts/make-fixtures.ts to reproduce them without publishing a
    // household's minute-by-minute water use.
    //
    // What this can no longer prove: that the estimator still agrees with the
    // physical meter. `npm run verify:prod` does that against live data, on
    // demand, locally. This proves the estimator still decomposes a known
    // signature the same way — which is what a regression test is for.
    ...
    const rows: FlumeRow[] = JSON.parse(
      readFileSync(join(process.cwd(), "data", "fixture-sprinkler-day.json"), "utf8")
    ).rows
    ...
    //
    // The assertion is on the DIFFERENCE, not the two absolute drifts. Where the
    // tail stations sit within a couple of gpm of each other, reconcileDay's
    // "largest flow step" refinement can settle on either edge of a one-minute
    // gap, so the absolute boundary carries ±2 min of tie-breaking noise — it
    // was 16→6 on the real meter and is 18→8 here. What is invariant, and what
    // the fix actually claims, is that configuring the delay removes exactly the
    // delay's accumulated contribution: 60 s × 10 transitions.
    ...
    const driftBefore = before[before.length - 1].startDriftMin!
    const driftAfter = after[after.length - 1].startDriftMin!
    expect(driftBefore - driftAfter).toBe(10)
    // The residual is duration overrun, and it must NOT be absorbed by the delay.
    expect(driftAfter).toBeGreaterThan(0)
