...
| Flume credentials are never stored | The account password is read from the environment for each token exchange and nothing is persisted — no credentials table, no refresh token at rest, no encryption key to manage. The earlier design of this feature kept an encrypted refresh token in the database; not having the secret at all is the stronger position. |
